...
public class User_Others_Regsample_Oral {
	Logger logger = LogManager.getLogger(User_Others_Regsample_Oral.class);
	WebDriver driver = Instance.getInstance();
	Properties prop = PropertiesFile.readPropertyFile("User_Others_Regsample_Oral.properties");
	
	@Test(priority=9,enabled=true)
	public void RegisteringOralSampleForOthers() throws InterruptedException, IOException 
	{
		Thread.sleep(4000);
		JavascriptExecutor jse = (JavascriptExecutor) driver;
		logger.info("Others Oral sample registration process has been started");
	    ...
	    Thread.sleep(3000);
	    logger.info(" Others Oral sample registration process has been completed");
