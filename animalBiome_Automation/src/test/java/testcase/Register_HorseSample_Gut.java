...
public class Register_HorseSample_Gut {
	Logger logger = LogManager.getLogger(Register_HorseSample_Gut.class);
	WebDriver driver = Instance.getInstance();
	Properties prop = PropertiesFile.readPropertyFile("Register_HorseSample_Gut.properties");
	@Test(enabled = true)
	public void RegisteringTheHorseGutSample() throws InterruptedException, AWTException, IOException {
		logger.info("***** Gut sample of Horse registration process has been started*******");
		Thread.sleep(4000);
		...
		st.selectByIndex(1);
		Thread.sleep(2000);
		driver.findElement(By.xpath(prop.getProperty("N_Fecal_Cons"))).sendKeys(prop.getProperty("N_Fecal_cons_Text"));
		Thread.sleep(1500);
		...
		logger.info("***** Horse Gut Sample registration process has been completed******");
